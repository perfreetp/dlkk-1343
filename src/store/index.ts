...
export const useStore = create<StoreState>((set, get) => ({
  ...
  getEpisodesBySeason: (seasonId) => get().episodes.filter((e) => e.seasonId === seasonId).sort((a, b) => a.number - b.number),
  ...
  updateEpisodeStatus: (episodeId, status) =>
    set((state) => ({
      episodes: state.episodes.map((e) =>
        e.id === episodeId ? { ...e, status, updatedAt: new Date().toISOString().split('T')[0] } : e,
      ),
    })),
  ...
  addTopic: (topic) =>
    set((state) => ({
      topics: [
        ...state.topics,
        { ...topic, id: `t${Date.now()}`, createdAt: new Date().toISOString().split('T')[0], votes: 0, status: 'backlog' as const },
      ],
    })),
    ...
    set((state) => ({
      episodes: state.episodes.map((e) =>
        e.id === episodeId
          ? {
              ...e,
              reviews: e.reviews.map((r) => (r.id === commentId ? { ...r, resolved: true } : r)),
            }
          : e,
      ),
    })),
    ...
    set((state) => ({
      episodes: state.episodes.map((e) =>
        e.id === episodeId
          ? {
              ...e,
              mistakes: e.mistakes.map((m) => (m.id === mistakeId ? { ...m, fixed: !m.fixed } : m)),
            }
          : e,
      ),
    })),
